...
const PasswordForm = () => {
  ...
  const processForm: SubmitHandler<Inputs> = async (data) => {
    ...
    try {
      await resetPassword(searchParams.get("code")!, data.password);
    ...
    <div className="w-full">
      <form onSubmit={handleSubmit(processForm)}>
        <div className="relative">
          ...
          <Link
            className="absolute bottom-0 right-0 -mb-5 text-xs text-blue-500 hover:text-blue-600 hover:underline"
            href={`forgot-password?email=${searchParams.get("email")}`}
          >
            Forgot?
          </Link>
        </div>
