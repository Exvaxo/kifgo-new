...
import { isEmailAvailable } from "@/app/_actions";
...
const EmailCheckForm = () => {
  ...
  const { setRegisterError, fetchUserLoading } = useAuthStore();
  ...
  const processForm: SubmitHandler<Inputs> = async (data) => {
    ...
    const result = await isEmailAvailable(data);
    ...
    if (result === "user-exists") {
      ...
      let url = `/user/register?email=${data.email}&step=2`;
