...
export async function updateProductDb(
  data: StockYouShopType,
  product: Prisma.ProductGetPayload<{ include: { settings: true } }>,
) {
      ...
      data: {
        ...
        details: {
          update: {
            data: {
              attributes: {
                updateMany: data.details.attributes.map((attr) => {
                  return {
                    data: attr,
                    where: {
                      AND: [
        ...
        variation: {
          upsert: data.variation.map(
            (
              variant,
            ): Prisma.ProductVariationUpsertWithWhereUniqueWithoutProductInput => {
              let obj: any;
              obj = {
              ...
              return {
                create: obj,
                update: obj,
                where: { variationId: variant.variationId },
              };
            },
          ),
        },
